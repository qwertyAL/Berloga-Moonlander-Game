...
function DrawLevel(c, level) {
    ...
    switch (level) {
        ...
        case 0:
            ...
            c.fillStyle = "#FFFFFF";
            c.font = "100px Roboto";
            FillCenterText(c, "МЕЖПЛАНЕТНАЯ", 110)
            FillCenterText(c, "ПОЧТОВАЯ", 210)
            FillCenterText(c, "СЛУЖБА", 310)
            ...
            FillCenterText(c, "Ваша задача как почтальона — доставлять почту в отдаленные колонии.", 420);
            FillCenterText(c, "Используйте клавиши курсора или WASD, чтобы управлять своим почтовым посадочным модулем.", 450);
            FillCenterText(c, "Пробел для переключения звука", 480);
            break;
        ...
        case 9:
            ...
            c.font = "80px Arial";
            c.fillStyle = "#FFFFFFFF";
            FillCenterText(c, "Вы успешно", 150)
            FillCenterText(c, "доставили всю почту", 230)
            c.font = "50px Arial";
            FillCenterText(c, "" + Math.floor(ships * 500 + score) + " Баллов", 300)
            break
    }
}
